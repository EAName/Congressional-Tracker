...
def tagged_substantive_vote_count(conn: duckdb.DuckDBPyConnection) -> int:
    """RULE/HUMAN-tagged votes excluding site-suppressed categories."""
    suppressed = sorted(SITE_SUPPRESSED_CATEGORIES)
    ph = ", ".join("?" for _ in suppressed)
    row = conn.execute(
        f"""
...
def publication_funnel(conn: duckdb.DuckDBPyConnection) -> list[dict[str, Any]]:
    """
    Outreach funnel stages for the Sheets Dashboard.

    Stage labels are reader-facing; counts exclude procedural suppressions.
    """
    from vact.pipeline.notify import find_party_line_splits

    return [
        {
            "stage": "Tagged substantive votes",
            "count": tagged_substantive_vote_count(conn),
        },
        {
            "stage": "Party-line VA splits",
            "count": len(find_party_line_splits(conn)),
        },
        {
            "stage": "Narrative briefs ready",
            "count": publication_ready_count(conn),
        },
    ]


def build_outreach_stories(
    conn: duckdb.DuckDBPyConnection,
    *,
    limit: int = 12,
    map_version: str = "2026",
    ...
    """
    Ranked story queue for caucus outreach.

    Start from party-line tagged splits, score higher when Target Four members
    oppose the Dem caucus position and when a human summary already exists.
    """
    from vact.pipeline.notify import find_party_line_splits
    ...
    target_ids = [m["bioguide_id"] for m in targets]
    by_id = {m["bioguide_id"]: m for m in targets}
    ...
    for signal in signals:
        meta = conn.execute(
            ...
            SELECT
                coalesce(v.bill_id, ''),
                coalesce(b.title, v.vote_question, v.bill_id, v.vote_id),
                coalesce(b.plain_language_summary, ''),
                v.vote_category,
        ...
        bill_id = meta[0] if meta else ""
        title = str(meta[1] if meta else signal.vote_id)
        summary = str(meta[2] if meta else "").strip()
        category = str(meta[3] if meta else signal.vote_category)
        bill_type = str(meta[4] if meta else "").lower()
        # House/Senate resolutions are floor procedure, not outreach claims.
        ...
        if target_ids:
            ...
            pos_rows = conn.execute(
                f"""
                SELECT m.bioguide_id, m.position
                FROM fact_member_vote m
                WHERE m.vote_id = ?
                  AND m.bioguide_id IN ({ph})
                  AND m.position IN ('YEA', 'NAY')
                """,
                [signal.vote_id, *target_ids],
            ).fetchall()
            for bioguide_id, position in pos_rows:
                member = by_id.get(bioguide_id) or {}
                disagrees = position != signal.dem_position
                if disagrees:
                    disagree += 1
                target_positions.append(
                    {
                        "bioguide_id": bioguide_id,
                        "full_name": member.get("full_name") or bioguide_id,
                        "district_number": member.get("district_number"),
                        "position": position,
                        "disagrees_with_dems": disagrees,
                    }
                )
            target_positions.sort(key=lambda r: r.get("district_number") or 0)
        ...
        stories.append(
            {
                "vote_id": signal.vote_id,
                "vote_date": signal.vote_date,
                "vote_category": category,
                "bill_id": bill_id,
                "title": title,
                "short_title": short_title,
                "tags": list(signal.tags),
                "dem_position": signal.dem_position,
                "gop_position": signal.gop_position,
                "source_url": signal.source_url or "",
                "summary": summary,
                "summary_ready": bool(summary),
                "score": score,
                "target_disagree_n": disagree,
                "target_positions": target_positions,
            }
        )

    stories.sort(
        key=lambda s: (s["score"], s["vote_date"], s["vote_id"]),
        reverse=True,
    )
