...
export default function ScoreOverTime({
  ...
  themeLabel: string;
  /** Contested-vote floor from meta. Below it a cell is charted but not publishable. */
  ...
  return (
    <div className="time-chart">
      ...
      <p className="scale-cap">
        ...
        {/* The head-to-head marks thin cells; this chart drew them like any other, so
            the same number looked provisional in one module and settled in the next.
            A one-vote line is a point, not a trend. */}
        {sufficientMin != null && last.n < sufficientMin
          ? ` · below the ${sufficientMin}-vote floor, not publishable on its own`
          : ""}
      </p>
    </div>
  );
